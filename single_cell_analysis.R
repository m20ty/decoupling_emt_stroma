...
pdf(
    '../data_and_figures/simulated_bulk_lineplots_lenient.pdf',
    width = 9.5,
    height = 6
)
...
# Run a similar comparison for the TCGA deconvolution:
...
sc_tcga_deconv_comp <- sapply(
	...
	function(ct) {
		...
		deconv_filtered$cor_with_purity <- sapply(
			deconv_filtered$cor_with_purity,
			function(x) x[filtered_genes],
			simplify = FALSE,
			USE.NAMES = TRUE
		)
