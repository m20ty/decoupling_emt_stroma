commit 956a4552fcd99b554246fe048939bcd9aee856ca
Author: m20ty <m20tyler2@aol.com>
Date:   Sat Aug 8 11:51:39 2020 +0300

    Created git repository

diff --git a/deconv.R b/deconv.R
new file mode 100644
index 0000000..0618b1d
--- /dev/null
+++ b/deconv.R
@@ -0,0 +1,1731 @@
+library(data.table)
...
+                    !(names(deconv_args_per_ct[[ct]]) == 'plot_title')
+                ]
+            )
+        )
+        
...
+#     lapply(
+#         deconv_plots[
+#             sapply(deconv_plots, function(li) {length(li$plots) == 5})
+#         ],
+#         function(li) {
+#             list(plots = li$plots[c('heatmap', 'purity_bar', 'ccle_bar', 'extra_bar')])
+#         }
+#     ),
+#     n_row = 8,
+#     n_col = 4
...
+
+pdf(
+    '../data_and_figures/deconv_figures_purity_ccle_sc.pdf',
+    width = 10,
+    height = 12
+)
+
+lapply(
+    deconv_plots,
+    function(deconv_plot_ct) {
+        if(length(deconv_plot_ct$plots) == 5) deconv_plot(list(deconv_plot_ct))
+    }
+)
+
+# i <- 1
...
+#         
+#         ggarrange(
+#             plots = deconv_ct$plots,
+#             ncol = 1,
+#             nrow = 5,
+#             heights = c(8, 1.6, 0.8, 0.8, 0.8),
...
+
+pdf(
+    '../data_and_figures/deconv_figures_purity_ccle.pdf',
+    width = 10,
+    height = 11.2
+)
+
...
+    deconv_plots,
+    function(deconv_plot_ct) {
+        if(length(deconv_plot_ct$plots) == 4) deconv_plot(list(deconv_plot_ct))
+    }
+)
+
+# i <- 1
...
+
+pdf(
+    '../data_and_figures/deconv_figures_purity.pdf',
+    width = 10,
+    height = 10.4
+)
+
...
+    deconv_plots,
+    function(deconv_plot_ct) {
+        if(length(deconv_plot_ct$plots) == 3) deconv_plot(list(deconv_plot_ct))
+    }
+)
+
+# i <- 1
...
+
+pdf(
+    '../data_and_figures/deconv_diagnostics.pdf',
+    width = 10,
+    height = 12
+)
+
...
+    ggarrange(
+        plots = c(
+            list(deconv_plot_ct$plots$heatmap),
+            deconv_plot_ct$diagnostics$alternative_purity_cor,
+            deconv_plot_ct$diagnostics$cell_type_bars
+        ),
+        ncol = 1,
+        nrow = 11,
