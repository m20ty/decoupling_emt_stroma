...
subtypes_data <- fread('../../TCGA_data/tcga_subtypes_data_simplified.csv', key = 'id')
...
saveRDS(deconv_data, '../data_and_figures/deconv_simplified_data.rds')
saveRDS(deconv_plots, '../data_and_figures/deconv_simplified_plots.rds')
...
pdf('../data_and_figures/deconv_simplified_figures.pdf', width = 10, height = 12)
...
# Write all figures to PDFs, depending on how much extra data they have:





# Figures with CCLE and single cell data:

# deconv_plot(
    # lapply(
        # deconv_plots[sapply(deconv_plots, function(li) {length(li$plots) == 5})],
        # function(li) list(plots = li$plots[c('heatmap', 'purity_bar', 'ccle_bar', 'extra_bar')])
    # ),
    # n_row = 8,
    # n_col = 4
# )

# pdf('../data_and_figures/deconv_figures_purity_ccle_sc.pdf', width = 10, height = 12)

# lapply(deconv_plots, function(deconv_plot_ct) {if(length(deconv_plot_ct$plots) == 5) print(deconv_plot(list(deconv_plot_ct), legends_space = 0.2))})

# # i <- 1
# # for(deconv_plot_ct in deconv_plots) {
    # # if(length(deconv_plot_ct$plots) == 5) {
        # # ggarrange(plots = deconv_ct$plots, ncol = 1, nrow = 5, heights = c(8, 1.6, 0.8, 0.8, 0.8), newpage = switch((i == 1) + 1, TRUE, FALSE))
        # # i <- i + 1
    # # }
# # }
...
# Figures with CCLE data but no single cell data:

# pdf('../data_and_figures/deconv_figures_purity_ccle.pdf', width = 10, height = 11.2)

# lapply(deconv_plots, function(deconv_plot_ct) {if(length(deconv_plot_ct$plots) == 4) print(deconv_plot(list(deconv_plot_ct), legends_space = 0.2))})

# # i <- 1
# # for(deconv_plot_ct in deconv_plots) {
    # # if(length(deconv_plot_ct$plots) == 4) {
        # # ggarrange(plots = deconv_ct$plots, ncol = 1, nrow = 4, heights = c(8, 1.6, 0.8, 0.8), newpage = switch((i == 1) + 1, TRUE, FALSE))
        # # i <- i + 1
    # # }
# # }
...
# Figures with only purity data:

# pdf('../data_and_figures/deconv_figures_purity.pdf', width = 10, height = 10.4)

# lapply(deconv_plots, function(deconv_plot_ct) {if(length(deconv_plot_ct$plots) == 3) print(deconv_plot(list(deconv_plot_ct), legends_space = 0.2))})

# # i <- 1
# # for(deconv_plot_ct in deconv_plots) {
    # # if(length(deconv_plot_ct$plots) == 3) {
        # # # deconv_plot(deconv_plot_ct)
        # # ggarrange(plots = deconv_ct$plots, ncol = 1, nrow = 3, heights = c(8, 1.6, 0.8), newpage = switch((i == 1) + 1, TRUE, FALSE))
        # # i <- i + 1
    # # }
# # }
...
pdf('../data_and_figures/deconv_simplified_diagnostics.pdf', width = 10, height = 12)
...
# An example of one deconvolution:
...
# deconv_ct <- do.call(
    ...
    # args = c(
        # list(
            ...
            # extra_data = extra_data,
            # initial_gene_weights = FALSE
            # # purity_colour_limits = c(-0.2, 0.2),
            # # purity_legend_breaks = c(-0.2, 0, 0.2)
        # ),
        # deconv_args_per_ct[[ct]]
    # )
# )

# pdf(
    # '../data_and_figures/deconv_brca.pdf',
    # width = 7.5,
    # height = 7
# )

# ggarrange(
    # deconv_ct$plots$purity_bar +
        # labs(
            # y = 'Correlation with purity',
            # title = 'Breast cancer'
        # ) +
        # scale_y_discrete(
            # expand = c(0, 0),
        ...
        # theme(
            ...
            # legend.margin =  margin(-3, 1, 1, 5.5),
            # legend.text = element_text(vjust = -0.4),
            # axis.title.y.right = element_text(angle = 0, vjust = 0.5),
            # plot.title = element_text(size = 22),
            # plot.margin = unit(c(5.5, 5.5, 1, 5.5), 'pt')
        # ),
    # deconv_ct$plots$ccle_bar +
        # labs(y = 'Tumours vs. cell lines') +
        # scale_y_discrete(
            # expand = c(0, 0),
            # position = 'right'
        # ) +
        # theme(
            # legend.position = 'left',
            # legend.margin = margin(1, 1, -4.5, 5.5),
            # legend.text = element_text(vjust = -0.4),
            # axis.title.y.right = element_text(angle = 0, vjust = 0.5),
            # plot.margin = unit(c(1, 5.5, 1, 5.5), 'pt')
        # ),
    # deconv_ct$plots$extra_bar +
        # labs('scRNA-seq') +
        # scale_y_discrete(
            # expand = c(0, 0),
            # position = 'right'
        ...
        # theme(
            # legend.position = 'left',
            # legend.margin = margin(1, 1, -12, 5.5),
            # legend.text = element_text(vjust = -0.4),
            # axis.title.y.right = element_text(angle = 0, vjust = 0.5),
            # plot.margin = unit(c(1, 5.5, 1, 5.5), 'pt')
        # ),
    # deconv_ct$plots$heatmap +
        # theme(
            # plot.title = element_blank(),
            # plot.margin = unit(c(1, 5.5, 0, 5.5), 'pt')
        # ),
    # deconv_ct$plots$axis_labels,
    # ncol = 1,
    # nrow = 5,
    # heights = c(0.5, 0.5, 0.5, 10, 1.6),
    # newpage = FALSE
# )
...
# A selection for small, unannotated versions:

# With scRNA-seq: LUSC, PAAD, READ (maybe not HNSC because I'm already showing the single
# cell heatmap for head and neck).

# Without scRNA-seq: BLCA, ESCA, UCEC
...
# pdf(
    # '../data_and_figures/deconv_mini_examples.pdf',
    # width = 5,
    # height = 4
# )
...
# ggarrange(
    # plots = c(
        # lapply(
            # c('lusc', 'paad', 'read'),
            # function(ct) {
                # deconv_plots[[ct]]$plots$purity_bar +
                    # labs(title = titles[[ct]]) +
                    # theme(
                        # plot.margin = unit(c(2.5, 2.5, 1, 2.5), 'pt'),
                        # axis.title.y = element_blank(),
                        # legend.position = 'none',
                        # plot.title = element_text(margin = unit(c(0, 5.5, 1, 5.5), 'pt'))
                    # )
            # }
        # ),
        # lapply(
            # c('lusc', 'paad', 'read'),
            # function(ct) {
                # deconv_plots[[ct]]$plots$ccle_bar +
                    # theme(
                        # plot.margin = unit(c(0, 2.5, 1, 2.5), 'pt'),
                        # axis.title.y = element_blank(),
                        # legend.position = 'none'
                    # )
            # }
        ...
        # lapply(
            # c('lusc', 'paad', 'read'),
            # function(ct) {
                # deconv_plots[[ct]]$plots$extra_bar +
                    # theme(
                        # plot.margin = unit(c(0, 2.5, 1, 2.5), 'pt'),
                        # axis.title.y = element_blank(),
                        # legend.position = 'none'
        ...
        # lapply(
            # c('lusc', 'paad', 'read'),
            # function(ct) {
                # deconv_plots[[ct]]$plots$heatmap +
                    # theme(
                        # title = element_blank(),
                        # plot.margin = unit(c(0, 2.5, 5.5, 2.5), 'pt'),
                        # axis.title.y = element_blank(),
                        # legend.position = 'none'
                    # )
            # }
    ...
    # ncol = 3,
    # nrow = 7,
    # heights = c(0.5, 0.5, 0.5, 10, 0.5, 0.5, 10)
# )
