...
# This was an attempt to see if simplifying the deconv arguments, by always using 200 genes and gene weights function the 90th percentile,
# would give decent results.  It was not as good as the original analysis with optimised parameters.

# EDIT: I've changed this, and I'm having another go with a couple of different aspects.  First, I'm standardising the quantile function to
# use for gene filtering but I'm trying 3 different thresholds for the number of genes included in the figure: 150, 200 and 250.  Secondly,
# I'm not using the inferred subtypes: I'm only using samples to which a TCGA subtype has already been assigned.  This was because I found
# problems with the method for inferring subtypes - if I'd wanted to make them more robust, I would be left with basically the same number
# of tumours that already had assigned subtypes.
...
# deconv_data <- lapply(
# 	list(list(gene_weights_fun = function(x) quantile(x, 0.9)), list(gene_weights_fun = median), list(cell_type_weights = FALSE)),
# 	function(weights_arg) setNames(
# 		lapply(
# 			c(150, 200, 250),
# 			function(n) sapply(
# 				names(deconv_args_per_ct),
# 				function(ct) {
# 					cat(paste0(ct, ', weights arg = ', deparse(quote(weights_arg[[1]])), ', n = ', n, '\n'))
# 					deconv_ct <- do.call(
# 						deconvolve_emt_caf_data,
# 						args = c(
# 							list(
...
# 		c('n = 150', 'n = 200', 'n = 250')
# 	)
# ) %>% setNames(c('90th percentile', 'median', 'none'))

deconv_data <- unlist(unlist(deconv_data, recursive = FALSE), recursive = FALSE)

names(deconv_data) <- gsub('\\.', ', ', names(deconv_data))
# names(deconv_data) <- apply(str_split_fixed(names(deconv_data), '\\.', 3), 1, function(r) paste(r[3], r[1], r[2], sep = ', '))
# deconv_data <- deconv_data[order(names(deconv_data))]
...
saveRDS(deconv_data, '../data_and_figures/deconv_alt_data_all.rds')
saveRDS(deconv_plots, '../data_and_figures/deconv_alt_plots_all.rds')
...
pdf('../data_and_figures/deconv_alt_figures_all.pdf', width = 10, height = 12)
for(deconv_plot_ct in deconv_plots) {print(deconv_plot(list(deconv_plot_ct), legends_space = 0.2))}
dev.off()

# Diagnostic figures:
# pdf('../data_and_figures/deconv_alt_diagnostics_all.pdf', width = 10, height = 12)
# i <- 1
...
# 						genes_from_tcga_fun = function(x) top_cols_by_fun_cor(x, initial = initial_genes, threshold_fun = function(x) quantile(x, 0.99))$id,
# 						initial_gene_weights = FALSE,
# 						genes_filter_fun = function(x) 1:i,
...
# 	)
#
# 	saveRDS(deconv_data, paste0('../data_and_figures/deconv_simplified_false_', i, '_data.rds'))
# 	saveRDS(deconv_plots, paste0('../data_and_figures/deconv_simplified_false_', i, '_plots.rds'))
#
#
#
#
#
# 	# Write all figures to a single PDF:
# 	pdf(paste0('../data_and_figures/deconv_simplified_false_', i, '_figures.pdf'), width = 10, height = 12)
# 	for(deconv_plot_ct in deconv_plots) {print(deconv_plot(list(deconv_plot_ct), legends_space = 0.2))}
# 	dev.off()
#
# 	# Diagnostic figures:
# 	pdf(paste0('../data_and_figures/deconv_simplified_false_', i, '_diagnostics.pdf'), width = 10, height = 12)
# 	i <- 1
# 	for(deconv_plot_ct in deconv_plots) {
# 		ggarrange(
# 			plots = c(list(deconv_plot_ct$plots$heatmap), deconv_plot_ct$diagnostics$alternative_purity_cor, deconv_plot_ct$diagnostics$cell_type_bars),
